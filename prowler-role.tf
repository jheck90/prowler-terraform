...
data "aws_iam_policy_document" "prowler_assume_role_policy" {
  statement {
    ...
    condition {
      ...
      values = [
        "arn:${data.aws_partition.current.partition}:iam::${data.aws_caller_identity.current.account_id}:${var.iam_principal}",
      ]
    }
  }
}
...
resource "aws_iam_policy" "prowler_scan_policy" {
  ...
  policy      = file("${path.module}/prowler-policy.json")
}
...
resource "aws_iam_role_policy_attachment" "prowler_assume_role_attachment" {
